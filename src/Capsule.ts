...
import { createSSHServer } from "./server/exports"
import type { SSHServerInstance } from "./server/exports"
    ...
    return {
        ...
        async shutdown(): Promise<void> {
            ...
            try {
                ...
                // Shutdown SSH server if it exists
                if (sshServer) {
                    try {
                        await sshServer.shutdown()
                    } catch (error) {
                        // Log but don't fail shutdown
                        console.error("Error shutting down SSH server:", error)
                    }
                    sshServer = null
                }
